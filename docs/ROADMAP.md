...
This document preserves the testing-phase design and future expansion plan. Some testing-phase features are already implemented; others remain planned. See the [README](../README.md) for current capabilities and the [development notes](DEVELOPMENT.md) for implementation details.
...
Original repository: `NCU_IM_CapstoneProject` (the separate 2019 Android project).
...
The current build's balances and cost-basis/rounding rules are documented in the [README](../README.md#accounting-rules). Real-data scenarios must also define corporate-action eligibility before activation.
